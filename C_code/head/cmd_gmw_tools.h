...
typedef struct _console_graphics_info_ {
	...
	/*注意在自写工具函数中，该位置为框架（色块）页面的左上角位置！！！*/
	int start_x;
	int start_y;

	/* cmd窗口的大小 */
	int lines;		//为了给中文输入法提示行及运行结束的提示信息留空间，要求在计算得到的结果基础上（上/下额外空间+上状态栏+列标显示+主区域+下状态栏）+ 4（1中文输入法提示行+3预留空行）
	int cols;
...
int gmw_set_font(CONSOLE_GRAPHICS_INFO *const pCGI, const char *fontname = "Terminal", const int fs_high = 16, const int fs_width = 8);//缺省为点阵字体/16-8
...
/* 画一个色块 */
int gmw_draw_block(const CONSOLE_GRAPHICS_INFO *const pCGI, const int row_no, const int col_no, const int bdi_value, const BLOCK_DISPLAY_INFO *const bdi);//编号为从0开始！！！
/* 移动一个色块 */
int gmw_move_block(const CONSOLE_GRAPHICS_INFO *const pCGI, const int row_no, const int col_no, const int bdi_value, const int blank_bdi_value, const BLOCK_DISPLAY_INFO *const bdi, const int direction, const int distance);//编号从0开始！！！
/* 读键盘和鼠标 */
int gmw_read_keyboard_and_mouse(const CONSOLE_GRAPHICS_INFO *const pCGI, int &MAction, int &MRow, int &MCol, int &KeyCode1, int &KeyCode2, const bool update_lower_status_line = true);//编号从0开始！！！
